...
**レスポンシブ要件 [USER-REQ](2026-07-17追加)**:
- 全画面がモバイル(幅390px基準、下限360px)で操作可能であること
- 共通サイドバー: md未満ではハンバーガーメニュー(オーバーレイドロワー)に切り替え。Reviewバッジはハンバーガーアイコン上にも表示
- パイプライン(SCR-005): ステージ列は横スクロール(スナップ付き)。DnDはモバイルでは必須としない(ステージ変更はディール詳細のselectで代替可能なため)
- テーブル(SCR-007/008/013): md未満ではカード型リスト表示に切り替え、横スクロールを発生させない
- チャット(SCR-004)・承認キュー(SCR-010): 1カラム全幅。入力欄・ボタンのタップターゲットは44px以上
- 検証: E2E-019(モバイルビューポート)をP0に追加
